...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <% 
      ...
      <br /><p>
  Question No. <%: i+1 %> </p><p> Question ID <%: my_q.ID%></p><p><%: my_q.Text%></p><br /> <p>The Question Wight is <%: my_q.Wight%></p><br />
       ...
       <% for (int j = 0; j < a_loop; j++)
          {
              ...
              <%: Html.HiddenFor(m => m[i].A[j].is_Right)%>
               ...
               <%: Html.HiddenFor(m => m[i].A[j].Q_ID)%>
                <%: Html.HiddenFor(m => m[i].A[j].Text)%>
                 <%: Html.HiddenFor(m => m[i].A[j].value)%>
             <div style="background-color:#CCFFCC; color:Black ; font-weight: bold; font-size:medium; text-indent: 2.0em"><p>Answer No. <%: j + 1 %></p><%: ans.Text%></div></div><br />
       <% } %>
...
<div style="background-color:#8C0D1B; color:White ; font-weight: bold; font-size:medium; text-indent: 2.0em"><br /><%: my_al.Text %> <br /></div><br />
